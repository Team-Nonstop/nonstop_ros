...
#include "nonstop_ar_pose/nonstop_ar_multi.h"
#include "nonstop_ar_pose/object.h"

int main (int argc, char **argv)
{
  ros::init (argc, argv, "ar_multi");
  ros::NodeHandle n;
  nonstop_ar_pose::ARMultiPublisher ar_multi (n);
  ros::spin ();
  return 0;
}

namespace nonstop_ar_pose
{
  ARMultiPublisher::ARMultiPublisher (ros::NodeHandle & n):n_ (n), it_ (n_)
  {
    ...
    if (!n_param.getParam ("camera_name", cameraName_))
      cameraName_ = "/camera";
    ROS_INFO ("\tCamera Name: %s", &cameraName_);
    ...
    // **** subscribe
    cameraImageTopic_ = cameraName_ + "/image_raw";
    cameraInfoTopic_  = cameraName_ + "/camera_info";
    ...
    arMarkerPub_ = n_.advertise < nonstop_ar_pose::ARMarkers > ("ar_pose_marker", 0);
...
// DEPRECATED: Fuerte support ends when Hydro is released
  ...
  void ARMultiPublisher::getTransformationCallback (const sensor_msgs::ImageConstPtr & image_msg)
  {
    ...
    for (i = 0; i < objectnum; i++)
    {
      ...
      nonstop_ar_pose::ARMarker nonstop_ar_pose_marker;
      nonstop_ar_pose_marker.header.frame_id = image_msg->header.frame_id;
      nonstop_ar_pose_marker.header.stamp = image_msg->header.stamp;
      nonstop_ar_pose_marker.id = object[i].id;

      nonstop_ar_pose_marker.pose.pose.position.x = pos[0];
      nonstop_ar_pose_marker.pose.pose.position.y = pos[1];
      nonstop_ar_pose_marker.pose.pose.position.z = pos[2];

      nonstop_ar_pose_marker.pose.pose.orientation.x = quat[0];
      nonstop_ar_pose_marker.pose.pose.orientation.y = quat[1];
      nonstop_ar_pose_marker.pose.pose.orientation.z = quat[2];
      nonstop_ar_pose_marker.pose.pose.orientation.w = quat[3];

      nonstop_ar_pose_marker.confidence = round(marker_info->cf * 100);
      arPoseMarkers_.markers.push_back (nonstop_ar_pose_marker);
